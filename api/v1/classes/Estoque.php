...
class Estoque{ //CRIANDO A CLASSE
	...
	{
		try { //Tentanto executar os códigos abaixo
			...
			while ($row = $sql->fetch(PDO::FETCH_ASSOC)) { //ENQUANTO HOUVER RETORNO DE DADOS | fetch(): Retorna uma unica linha da consulta | jÁ O PDO::FETCH_ASSOC SERVE PARA RETORNA SOMENTE 1 CAMPO NO RETORNO PARA A VARIÁVEL
		...
		} catch (\Throwable $th) { //Senão conseguir executar os códigos acima
			echo "Deu erro";
		}
